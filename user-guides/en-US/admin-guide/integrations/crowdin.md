...
Click on _Configure >_ `Allow connection`
...
Access your Crowdin account _Settings > oAuth._ Click  `New Application`
...
Click `Create`, then _Edit_ in _Action_ to capture the _Client Credentials_.
...
Paste the _Client ID_ and _Client secret_ in Step 3 on the Meeds tab
...
`Save`. Your users can now [bind their Crowdin and Meeds accounts](../../user-guide/setting-up-your-account/linking-your-accounts/crowdin.md) from their user settings.
...
Now go back to your Crowdin _Settings_ > _API_ tab, then click the `New Token` button and select the scopes:&#x20;
...
Click `Create`. If requested, enter your Crowdin password and confirm, then copy the _Access Token_ string.&#x20;
